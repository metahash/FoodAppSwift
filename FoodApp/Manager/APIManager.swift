...
class APIManager {
    ...
    func requestServer(_ method: HTTPMethod, _ path: String, _ params: [String: Any]?, _ endcoding: ParameterEncoding, _ completionHandler: @escaping (JSON) -> Void) {
        let url = baseURL?.appendingPathComponent(path)
        // refreshTokenIfNeed {
    ...
    func createOrder(stripeToken: String, completionHandler: @escaping (JSON) -> Void) {
        ...
        if JSONSerialization.isValidJSONObject(jsonArray) {
            ...
            do {
                ...
                print("JSON serialization fail \(error)")
            }
        }
    }
    ...
    func completeOrder(orderId: Int, completionHandler: @escaping (JSON) -> Void) {
        let path = "api/driver/location/complete/"
        let params: [String: Any] = [
            "order_id": "\(orderId)",
            "access_token": self.accessToken
        ]
